...
// Partner Card Hover Delay
var initPartnerCardHover = function(){
    var hoverTimeouts = {};
    
    $(document).on('mouseenter', '.partner-card', function() {
        var $card = $(this);
        var cardIndex = $card.closest('.partner-slot').data('partner');
        
        // Clear any existing timeout for this card
        if (hoverTimeouts[cardIndex]) {
            clearTimeout(hoverTimeouts[cardIndex]);
            delete hoverTimeouts[cardIndex];
        }
        
        // Add hover class immediately
        $card.addClass('partner-card-hovered');
    });
    
    $(document).on('mouseleave', '.partner-card', function() {
        var $card = $(this);
        var cardIndex = $card.closest('.partner-slot').data('partner');
        
        // Set timeout to remove hover class after 5 seconds
        hoverTimeouts[cardIndex] = setTimeout(function() {
            $card.removeClass('partner-card-hovered');
            delete hoverTimeouts[cardIndex];
        }, 5000);
    });
};
initPartnerCardHover();
